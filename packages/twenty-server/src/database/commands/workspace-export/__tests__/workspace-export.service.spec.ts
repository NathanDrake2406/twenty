...
class MockWorkspaceScopedEntity extends WorkspaceRelatedEntity {}

class MockNonWorkspaceScopedEntity {}
...
function makeEntityMetadata({
  ...
  columns = [],
  manyToOneRelations = [],
  target = MockNonWorkspaceScopedEntity,
}: {
  tableName: string;
...
describe('WorkspaceExportService', () => {
  it('discovers direct, forward, and required reverse relations', () => {
    ...
    const userWorkspace = makeEntityMetadata({
      ...
      target: MockWorkspaceScopedEntity,
      columns: [
